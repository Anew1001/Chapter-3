...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        scaleXAnimator.setRepeatCount(ObjectAnimator.INFINITE); // 设置循环播放
        ObjectAnimator scaleYAnimator = ObjectAnimator.ofFloat(target, "scaleY", 1f, 2f);
        scaleYAnimator.setRepeatCount(ObjectAnimator.INFINITE); // 设置循环播放
        ...
        alphaAnimator.setRepeatCount(ObjectAnimator.INFINITE); // 设置循环播放
        alphaAnimator.setDuration(1000); // 设置动画持续时间为1秒
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,alphaAnimator, scaleXAnimator, scaleYAnimator);
        animatorSet.start();
    }
}
